...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Try to fetch generated data from public folder (created by Make/Notion)
  ...
  return (
    <div className="relative w-screen h-screen bg-slate-900 flex flex-col border-t-4 border-amber-400">
      
      {/* Top Overlay: Controls */}
      <div className="absolute top-0 left-0 right-0 z-10 pt-3 pointer-events-none">
        <header className="text-center mb-1 pointer-events-auto">
          <h1 className="text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-amber-100 to-amber-400 tracking-[0.2em] font-['Outfit'] drop-shadow-lg cursor-pointer" onClick={() => window.location.reload()}>
            FOOD'MATCH
          </h1>
      ...
      {/* Toast Notification for Errors/Leaf Nodes */}
      {notification && (
        <div className="absolute bottom-10 left-1/2 transform -translate-x-1/2 bg-slate-800 text-white px-6 py-3 rounded-full shadow-2xl border border-slate-600 animate-bounce z-50 flex items-center gap-2">
      ...
      <div className="absolute bottom-2 right-4 z-10 pointer-events-auto text-right flex flex-col items-end gap-1">
